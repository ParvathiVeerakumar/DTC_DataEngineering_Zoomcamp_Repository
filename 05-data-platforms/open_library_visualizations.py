...
def _():
    ...
    return alt, dlt, ibis, load_package_viewer, render
...
@app.cell
async def _(load_package_viewer, pipeline, render):
    await render(load_package_viewer(pipeline.working_dir))
    return
...
def _(alt, ibis, ibis_con):
    ...
    author_table = ibis_con.table("harry_potter_books__author_name")
    author_query = (
        author_table
        .group_by("value")
...
def _(alt, ibis_con):
    ...
    books_table = ibis_con.table("harry_potter_books")
    year_query = (
        books_table
...
def _(alt, ibis, ibis_con):
    ...
    lang_table = ibis_con.table("harry_potter_books__language")
    lang_query = (
        lang_table
        .group_by("value")
...
def _(ibis_con, mo):
    ...
    total_books = ibis_con.table("harry_potter_books").count().to_pandas()
    total_authors = ibis_con.table("harry_potter_books__author_name").value.nunique().to_pandas()
    total_languages = ibis_con.table("harry_potter_books__language").value.nunique().to_pandas()
